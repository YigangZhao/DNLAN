...
class FlyNonLocalSparseAttention(nn.Module):
    def __init__(self, n_hashes=4, channels=64, k_size=3, reduction=4, chunk_size=144, conv=common.default_conv, res_scale=1, hash_length=0, debug=False, layer=-1):
        super(FlyNonLocalSparseAttention, self).__init__()
        self.chunk_size = chunk_size
        self.n_hashes = n_hashes
        self.reduction = reduction
        self.res_scale = res_scale
    ...
    def fly_lsh(self, hash_length, x):
        sampling_ratio = 0.1
        threshold = 0
        ...
        data = x - x.mean(dim=-1).reshape(N, -1, 1)

        weights_shape = (1, self.n_hashes, data.shape[-1], embedding_size)  # [1, n_hashes, C, embedding_size]
        weights = torch.rand(weights_shape, dtype=x.dtype, device=device).expand(N, -1, -1, -1)  # [N, n_hashes, C, embedding_size]
        weights = (weights > 1 - sampling_ratio).float()

        all_activations = torch.einsum('btf,bnfe->bnte', x, weights)
        all_activations = all_activations.reshape((N, self.n_hashes, -1, hash_length, K)).sum(axis=-1)  # [N, n_hashes, H * W, hash_length]
        hashes = (all_activations > threshold).float()  # [N, n_hashes, H*W, hash_length]

        code = list()
        for i in range(hash_length):
            code.append(1 << i)
        code = torch.tensor(code, dtype=x.dtype, device=device)

        hash_codes = torch.matmul(hashes, code)  # [N,n_hashes,H*W]

        # add offsets to avoid hash codes overlapping between hash rounds
        offsets = torch.arange(self.n_hashes, device=device)
        hash_buckets = 1 << hash_length
        offsets = torch.reshape(offsets * hash_buckets, (1, -1, 1))
        hash_codes = torch.reshape(hash_codes + offsets, (N, -1,))  # [N,n_hashes*H*W]
    ...
    def forward(self, input):
        ...
        if self.hash_length <= 0:
            ...
            hash_length = 1
            while True:
                if (1 << hash_length) >= hash_buckets:
                    break
                hash_length += 1
        else:
            hash_length = self.hash_length

        hash_codes = self.fly_lsh(hash_length, x_embed)  # [N,n_hashes*H*W]
        hash_codes = hash_codes.detach()
